-- Taken from https://github.com/vincentsarago/tipg-aurora/blob/main/sql/dbcatalog.sql

-- Custom version of tipg dbcatalog.sql (replace pgtemp. with public.)

CREATE OR REPLACE FUNCTION public.typ(t text) RETURNS text AS $$
...
CREATE OR REPLACE FUNCTION public.nspname(n oid) RETURNS text AS $$
    SELECT CASE WHEN n=pg_my_temp_schema() THEN 'public' ELSE nspname::text END
    FROM pg_namespace WHERE oid=n;
$$ LANGUAGE SQL STABLE;

CREATE OR REPLACE FUNCTION public.nspname(n regnamespace) RETURNS text AS $$
    SELECT public.nspname(n::oid);
$$ LANGUAGE SQL STABLE;

CREATE OR REPLACE FUNCTION public.tipg_pk(
    table_oid oid
) RETURNS text AS $$
    SELECT attname::text
    FROM
...
CREATE OR REPLACE FUNCTION public.tipg_properties(
    att pg_attribute
) RETURNS jsonb AS $$
DECLARE
    attname text := att.attname;
    attdescription text := col_description(att.attrelid, att.attnum);
    atttype text := public.typ(format_type(att.atttypid, null));
    attmin json;
    attmax json;
    srid int;
    geometry_type text;
...
CREATE OR REPLACE FUNCTION public.tipg_tproperties(
    c pg_class
) RETURNS jsonb AS $$
    WITH t AS (
        SELECT
            jsonb_agg(public.tipg_properties(a)) as properties
        FROM
            pg_attribute a
        WHERE
            attnum>0
        ...
        'entity', 'Table',
        'pk', public.tipg_pk(c.oid),
        'name', c.relname::text,
        'schema', public.nspname(c.relnamespace),
...
CREATE OR REPLACE FUNCTION public.tipg_tproperties(
    ...
    SELECT public.tipg_tproperties(pg_class) FROM pg_class WHERE oid=tabl::regclass;
$$ LANGUAGE SQL;

CREATE OR REPLACE FUNCTION public.tipg_fun_defaults(defaults pg_node_tree) RETURNS text[] AS $$
    WITH d AS (
...
CREATE OR REPLACE FUNCTION public.tipg_fproperties(
    p pg_proc
) RETURNS jsonb AS $$
DECLARE
    defaults text[];
...
BEGIN
    IF p.pronargdefaults > 0 AND p.pronargs > 0 THEN
        defaults :=
            ...
            || public.tipg_fun_defaults(p.proargdefaults)
        ;
    ELSE
    ...
    WITH t AS (
        SELECT
            public.typ(format_type(argtype, null)) as argtype,
            argmode,
            proargname,
            def,
            argnum
    ...
    RETURN jsonb_build_object(
        'entity', 'Function',
        'name', p.proname,
        'schema', public.nspname(p.pronamespace),
        'properties', properties,
...
CREATE OR REPLACE FUNCTION public.tipg_fproperties(
    ...
    SELECT public.tipg_fproperties(pg_proc) FROM pg_proc WHERE oid=func::regproc;
$$ LANGUAGE SQL;

CREATE OR REPLACE FUNCTION public.tipg_get_schemas(include text[] DEFAULT NULL, exclude text[] DEFAULT NULL) RETURNS SETOF oid AS $$
DECLARE
BEGIN
...
CREATE OR REPLACE FUNCTION public.tipg_catalog(
    ...
    WITH a AS (
        SELECT
            public.tipg_tproperties(c) as meta
        FROM pg_class c, public.tipg_get_schemas(schemas,exclude_table_schemas) s
        WHERE
            c.relnamespace=s
            AND relkind IN ('r','v', 'm', 'f', 'p')
            AND has_table_privilege(c.oid, 'SELECT')
            AND c.relname::text NOT IN ('spatial_ref_sys','geometry_columns','geography_columns')
            AND (exclude_tables IS NULL OR concat(public.nspname(relnamespace),'.',c.relname::text) <> ALL (exclude_tables))
            AND (tables IS NULL OR concat(public.nspname(relnamespace),'.',c.relname::text) = ANY (tables))

        UNION ALL

        SELECT
            public.tipg_fproperties(p) as meta
        FROM
            pg_proc p, public.tipg_get_schemas(schemas,exclude_function_schemas) s
        WHERE
            p.pronamespace=s
            AND proretset
            AND prokind='f'
            ...
            AND provariadic=0
            AND (exclude_functions IS NULL OR concat(public.nspname(pronamespace),'.', proname::text) <> ALL (exclude_functions))
            AND (functions IS NULL OR concat(public.nspname(pronamespace),'.', proname::text) = ANY (functions))
